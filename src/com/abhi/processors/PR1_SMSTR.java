...
public class PR1_SMSTR extends AbstractTask {
	...
	public void run() {
		try {
			...
			String table = schema + "." + (smsTr.getStatus() != null && smsTr.getStatus().equals("drop") ? "userDetails_s_ssfd" : "userDetails_s_ss");
			rows.add(LoggerUtil.getListAsCsvString(Arrays.asList(smsTr.getUserId(), smsTr.getMsgId(), smsTr.getAutomationId(), act[1], act[3])));
