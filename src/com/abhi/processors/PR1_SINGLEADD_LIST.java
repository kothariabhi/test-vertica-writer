...
public class PR1_SINGLEADD_LIST extends AbstractTask {
	...
	public void run() {
		try {
			...
			SingleAddList singleAddListRequest = (SingleAddList) LoggerUtil.getObjectFromJson(jsonPayload,
					SingleAddList.class);
			int existing = singleAddListRequest.getExisting();
			if (existing == 0) {
				singleUserADD(singleAddListRequest, jsonPayload, this.requestId, existing);
			} else {
				singleUserUpdate(singleAddListRequest, jsonPayload, this.requestId, existing);
			}
